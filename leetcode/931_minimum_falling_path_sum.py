...
# class Solution:  # very very slow
#     def minFallingPathSum(self, A: List[List[int]]) -> int:
#         size = len(A)  # = len(A[0])
#         def dfs(y, x, sum_):
#             if y == size: return sum_
#             return min(
#                 dfs(y+1, xx, sum_ + A[y][xx])
#                 for xx in range(x-1, x+2) if 0 <= xx < size)
#         return min(dfs(0, x, 0) for x in range(size))
...
#         best = float("inf")
#         size = len(A)  # = len(A[0])
#         stack = [(0, x, 0) for x in range(size)]
#         while stack:
#             y, x, s = stack.pop()
#             if y == size:
#                 # best = min(best, s)  # min() is slower
#                 if s < best: best = s
#             else:
#                 for xx in range(x-1, x+1):
#                     if 0 <= xx < size:
#                         stack.append((y+1, xx, s + A[y][xx]))
#         return best


class Solution:  # 92 ms (32.51%) - with DP, changing values in place
    def minFallingPathSum(self, A: List[List[int]]) -> int:
        size = len(A)
        # let's iterate row by row, going from the bottom up, and
        # store in place of the A values the minimum possible
        # to achieve up to now
        for y in range(2, size+1):
            for x in range(size):
                A[-y][x] += min(
                    [A[-y+1][x+i] for i in (-1, 0, 1) if 0 <= x+i < size]
                )
        return min(A[0])
...
class BasicTest(unittest.TestCase):
    def test_1(self):
        ...
        expected_output = 12
        output = Solution().minFallingPathSum(input_)
        self.assertEqual(output, expected_output)

    def test_2(self):
        input_ = [[-19,-1,-96,48,-94,36,16,55,-42,37,-59,6,-32,96,95,-58,13,-34,94,85],
